    ...
    def __init__(self, token, **kwargs):
        ...
        async def welcome_send(message):
            ...
            if await self._db_handler.add_user(message.chat.id, self._db_type):
                ...
                keyboard.add(
                    InlineKeyboardButton(text=B.Btn_as_is, callback_data=BTN_CANCEL)
                )
                ...
                self._user_inreract[message.chat.id] = {
                    "type": 2,
            ...
            if user := await self._db_handler.get_user_info(
                ...
                if user["admin"] == True:
                    ...
                    for u in all_users:
                        ...
                        try:
                            await self._bot.send_message(
                                u["id"], text_to_all, entities=message.entities
                            )
        ...
        async def slovo_reply_commands(message):
            ...
            # Назначаем кнопки
            keyboard = InlineKeyboardMarkup(row_width=1)
            button_sing = InlineKeyboardButton(
                text=B.Btn_signs, callback_data=f"{BTN_SIGN}{cmd[1:]}"
            )
            button_holy = InlineKeyboardButton(
                text=B.Btn_saints, callback_data=f"{BTN_HOLY}{cmd[1:]}"
            )
        ...
        @self._bot.callback_query_handler(
            func=lambda call: call.data[:5] in (BTN_SIGN, BTN_HOLY)
        )
        @B.Bot_Sender.except_log
        ...
        async def timezone_start_choice(message):
            ...
            if user := await self._db_handler.get_user_info(
                ...
                button_list = []
                button_list.append(
                    InlineKeyboardButton(text=B.Btn_change, callback_data=BTN_TZ_CHANGE)
                )
                button_list.append(
                    InlineKeyboardButton(text=B.Btn_nothing, callback_data=BTN_CANCEL)
                )
        ...
        async def timezone_change(call):
            ...
            keyboard = InlineKeyboardMarkup(row_width=1)
            keyboard.add(
                InlineKeyboardButton(text=B.Btn_undo, callback_data=BTN_CANCEL)
            )
        ...
        @self._bot.callback_query_handler(
            func=lambda call: call.data == BTN_ML_TIME_BACK
        )
        @B.Bot_Sender.except_log
        ...
        @self._bot.callback_query_handler(
            func=lambda call: call.data.startswith(BTN_ML_TURN_TO)
        )
        @B.Bot_Sender.except_log
        async def mailing_turn(call):
            ...
            if await self._db_handler.set_user_mailing(
                message.chat.id, self._db_type, turn_to
            ):
                text, keyboard = await self._make_mailing_choice(message.chat.id)
        ...
        @self._bot.callback_query_handler(
            func=lambda call: call.data[:13] == BTN_ML_TIME_EDIT
        )
        @B.Bot_Sender.except_log
        async def mailing_edit_choice(call):
            ...
            text += B.Mailing_time_first_quest.format(
                B.Mailing_time_next
                if call.data[13:] == BTN_ML_TOMORROW
            ...
            button_list.append(
                InlineKeyboardButton(text=B.Btn_back, callback_data=BTN_ML_TIME_BACK)
            )
            keyboard.add(*button_list)
            await self._bot.edit_message_text(
                text,
        ...
        @self._bot.callback_query_handler(
            func=lambda call: call.data[:16] == f"{BTN_ML_TIME_SET}{BTN_ON}"
        )
        @B.Bot_Sender.except_log
        ...
        @self._bot.callback_query_handler(
            func=lambda call: call.data[:17] == f"{BTN_ML_TIME_SET}{BTN_OFF}"
        )
        @B.Bot_Sender.except_log
        ...
        async def editing_cancel_edit(call):
            ...
            text = message.text[: message.text.rindex("\n")]
            await self._bot.edit_message_text(
                text, message.chat.id, message_id=message.message_id
            )
                ...
                if self._user_inreract[user_id]["type"] in (1, 2):
                    ...
                    if tz_num is not None:
                        ...
                        await self._bot.edit_message_reply_markup(
                            user_id, self._user_inreract[user_id]["msg"], None
                        )
    ...
    async def _make_mailing_choice(self, user_id):
        ...
        if user := await self._db_handler.get_user_info(user_id, self._db_type):
            ...
            if user["mailing"] != 1:
                button_list.append(
                    InlineKeyboardButton(
                        text=B.Btn_on, callback_data=f"{BTN_ML_TURN_TO}{BTN_ON}"
                    )
                )
            else:
                button_list.append(
                ...
                button_list.append(
                    InlineKeyboardButton(
                        text=B.Btn_off, callback_data=f"{BTN_ML_TURN_TO}{BTN_OFF}"
                    )
                )
            button_list.append(
                InlineKeyboardButton(text=B.Btn_nothing, callback_data=BTN_CANCEL)
